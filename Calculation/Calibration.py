...
def cal_dif(matrix_data, stand_data, light_number):
    """
    ...
    difference = []
    try:
        for row in range(0, light_number):
            dis = []
            for col in range(0, len(matrix_data[0])):
                temp_dis = matrix_data[row][col] - stand_data[col]
                dis.append(temp_dis)
...
def draw_pic(matrix_data, title, pic_num):
    ...
    for i in range(0, len(matrix_data)):
        plt.plot(n, matrix_data[i], linewidth=3, label='NO.'+str(i))
...
# 计算多项式参数的最终函数
def cal_args(inputPath, standardPath, outputPath, cali_num, light_num):
    # 检查文件路径是否存在
    ...
    # 根据文件名称，读取数据为一个二维列表（矩阵）
    filename = "Spectrum_"
    DATA_input = []
    DATA_stand = []
    for j in range(0, cali_num):
        temp_row = []
        temp_col = []
        for i in range(1, light_num+1):
            filename_temp = inputPath + filename + str(j) + "_" + str(i) + ".sps"
            temp_row = import_file(filename_temp)
            # print(temp_DATA)
            temp_col.append(temp_row)
        DATA_input.append(temp_col)
        # print(standardPath + filename + str(j) + ".sps")
        DATA_stand.append(import_file(standardPath + filename + str(j) + ".sps"))
        # print(DATA_stand)

    # 计算差值
    Diff = []
    temp_diff = []
    for i in range(0, cali_num):
        temp_diff = cal_dif(DATA_input[i], DATA_stand[i], light_num)
        Diff.append(temp_diff)
    # print(len(Diff[0]))

    # 开始拟合
    Polynomial_Coefficients = []  # 拟合的多项式系数
    for row in range(0, light_num): # 光纤个数
        temp_args = []
        for col in range(0, len(Diff[0][0])):   # 波段数，每个波段上都有一个差值，对三个光照强度下的差值进行拟合
            x = []
            y = []
            for j in range(0, cali_num):
                x.append(Diff[j][row][col])
                y.append(DATA_input[j][row][col])
            # print("x = ", x)
            # print("y = ", y)
            temp_args.append(np.polyfit(y, x, 2))   # 线性拟合，y = ax+b # 方法1：Numpy自带的多项式拟合函数
            # print("拟合的参数为：", temp_args[col])
        Polynomial_Coefficients.append(temp_args)  # 保存的多项式参数：a*x^2 + b*x + c中的a,b,c
        export_file(Polynomial_Coefficients[row], outputPath, "Light_" + str(row) + ".txt")

    # 计算定标结果
    result = []
    for i in range(0, 3):
        draw_pic(DATA_input[i], outputPath + "Raw Data " + str(i), i)
        temp_temp_result = []
        for row in range(0, light_num):
            temp_result = []
            for col in range(0, len(Polynomial_Coefficients[0])):
                temp_value = DATA_input[i][row][col]
                temp_args = Polynomial_Coefficients[row][col]
                func = np.poly1d(temp_args)
                temp_result.append(temp_value - func(temp_value ))
            temp_temp_result.append(temp_result)
        result.append(temp_temp_result)
    print("计算结束！")
    return result
